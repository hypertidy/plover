...
system.time({
  oo <- insideclipper::inside_clipper_loop(pts, lpx, lpy, lex)
})

wrld_simpl@proj4string@projargs[1] <- NA_character_
library(sp)
system.time(so <- over(SpatialPoints(pts), as(wrld_simpl, "SpatialPolygons")))


## sf st_contains
sf::sf_use_s2(FALSE)
sfpts <- sfheaders::sf_point(pts)
sfx <- sf::st_set_crs(sf::st_as_sf(wrld_simpl), NA)
system.time(sfi <- unlist(sf::st_contains(sfx, sfpts)))
